...
def main():
    ...
    """
    parser = argparse.ArgumentParser(description="Submit a LSST object from the Fink database to TNS")
    parser.add_argument(
        '-diaObjectId', type=str, default=None,
        help="LSST objectId")
    parser.add_argument(
    ...
    if args.diaObjectId is None:
        raise NotImplementedError("You need to provide a LSST objectId")
    ...
    for index, obj in enumerate(objects):
        ...
        ids.append(pdf['r:diaObjectId'].values[0])

    json_report = save_logs_and_return_json_report(
        name=args.diaObjectId,
        folder=args.outpath,
        ids=ids,
        report=report
    )
